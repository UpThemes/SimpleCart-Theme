...
function simplecart_trim_excerpt($text) {
	if ( '' == $text ) {
...
function simplecart_the_excerpt($deprecated = '') {
	global $post;
	$output = '';
...
function simplecart_excerpt_rss() {
	global $post;
	$output = '';
	$output = strip_tags($post->post_excerpt);
	if ( post_password_required($post) ) {
	...
	return apply_filters('simplecart_excerpt_rss', $output);

}

add_filter('simplecart_excerpt_rss', 'simplecart_trim_excerpt');

// create nice multi_tag_title
// Credits: Martin Kopischke for providing this code

function simplecart_tag_query() {

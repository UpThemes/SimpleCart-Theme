...
function thmfooter_theme_link() {
    $themelink = '<a class="theme-link" href="http://themeshaper.com/simplecart/" title="Thematic Theme Framework" rel="designer">Thematic Theme Framework</a>';
    return apply_filters('simplecart_theme_link',$themelink);
}
add_shortcode('theme-link', 'thmfooter_theme_link');	

function thmfooter_login_link() {
    if ( ! is_user_logged_in() )
        $link = '<a href="' . get_option('siteurl') . '/wp-login.php">' . __('Login','simplecart') . '</a>';
    else
    $link = '<a href="' . wp_logout_url($redirect) . '">' . __('Logout','simplecart') . '</a>';
